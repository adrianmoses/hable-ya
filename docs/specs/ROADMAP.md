...
| 049 | Auto-updating learner level from error/vocabulary signals (promotion/demotion driven by aggregate trends) | implemented | [049-learner-leveling](049-learner-leveling/spec.md) ([decision](049-learner-leveling/decision.md)) |
| 050 | Initial-level placement / calibration (derive starting CEFR level from first N sessions instead of config) | implemented | [049-learner-leveling](049-learner-leveling/spec.md) ([decision](049-learner-leveling/decision.md)) |
...
| 2026-04-25 | Spec 049-learner-leveling drafted on branch `spec-learner-leveling-049-050` (bundles #049 + #050: heuristic placement policy at end of session 1 from cold-start diagnostic ladder, end-of-session auto-promote/demote with asymmetric hysteresis K=3/K=4, `band_history` audit table, `is_calibrated` derived from history not session count, `replay_placement.py` regression CLI against the 25 cold_start fixtures with ≥0.75 accuracy / ≤0.20 MAE bar); #049 + #050 → in-progress. Confidence Low pending threshold-tuning spike. |
| 2026-04-26 | Spec 049-learner-leveling reworked: model-emitted per-turn `cefr_band` becomes the primary signal (replaces heuristic weighted-score). `log_turn` tool schema gains `cefr_band: enum[A1..C1]` + system prompt gains a `BAND_RUBRIC` section. Prompt-engineering only — no fine-tune in this slice (deferred follow-up if `replay_placement.py` doesn't clear ≥0.75 accuracy). Placement = modal band over diagnostic turns; leveling = rolling mean over last 3 sessions of turns. CI regression gate ships advisory until prompt-only quality is measured. |
| 2026-04-26 | Spec 049-learner-leveling implemented; #049 + #050 → implemented. Divergences from spec (captured in the decision record): band constants/helpers consolidated into a new `hable_ya/learner/bands.py` (eliminates 5 inline `band in {…}` checks plus duplicated `BAND_TO_FLOAT`/`_BAND_MIDPOINT` and `bucket_band`/`band_from_production_level`); `_fetch_recent_session_bands` collapsed into one CTE+JOIN; `replay_placement.py` calls a new public `render_cold_start_prompt(band)` from `pipeline/prompts/builder.py` (also fixed a latent bug where replay rendered at `fixture.true_band` instead of the runtime's `default_learner_band`). Live `replay_placement.py` measurement against the local llama.cpp endpoint is the open empirical question — CI gate stays advisory until that run decides blocking-vs-fine-tune-spec. 289 pytest passing; ruff + mypy clean. |
